...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: a.calendar.metaTitle, description: a.calendar.metaDescription };
}

export default async function AdminKalendarPage() {
  const { locale, lp, a } = await getAdminPage();
  if (!(await isAdminSession())) {
    redirect(adminLoginRedirect("/admin/kalendar", locale));
  }

  const events = [...(await readCalendarEvents())].sort(
    (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
  );
  const deleteLabels = { delete: a.delete, deleteFailed: a.deleteFailed, saving: a.saving, confirm: a.calendar.deleteConfirm };
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 sm:py-24">
      ...
      <h1 className="mt-3 font-[family-name:var(--font-display)] text-3xl tracking-[0.06em] text-slate-900 sm:text-4xl">
        {a.calendar.listTitle}
      </h1>
      <div className="mt-8">
        <Link
          href={lp("/admin/kalendar/novi")}
      ...
      <ul className="mt-12 space-y-3">
        {events.map((e) => (
          ...
          >
            <Link
              href={lp(`/admin/kalendar/${encodeURIComponent(e.id)}`)}
            ...
            <DeleteCalendarEventButton id={e.id} labels={deleteLabels} />
          </li>
        ))}
      </ul>
      {events.length === 0 ? <p className="mt-8 text-sm text-[var(--muted)]">{a.calendar.empty}</p> : null}
